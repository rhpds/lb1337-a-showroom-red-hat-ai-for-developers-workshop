...
- Responses from LLMs are not deterministic, we need to use different techniques to test LLM integrations.
- Embedding models can be used to test for the similarity of two strings
- How to add and run a new test to ensure an LLM response from a customer claim query is similar to an expected example.

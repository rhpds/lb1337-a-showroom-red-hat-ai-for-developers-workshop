= Parasol AI Developer Workflow - 50 minutes
...
quarkus.langchain4j.openai.parasol-email.base-url=http://parasol-instruct-predictor.aiworkshop.svc.cluster.local:8080/v1<4>
----
...
<4> Specify the base URL of the LLM inference endpoint that the application will connect to
...
This interface will prompt for an email sent from a customer, and give back the email response for the customer service rep. In other modules in this workshop, you'll see how you can use other techniques to automatically send this email! But for now we'll stick to copy/paste.
...
Before committing, do a quick sanity build of the project to ensure you got all the changes correct. Run the following command in the Terminal:

[.console-input]
[source,shell,subs="+attributes,macros+"]
----
mvn -f $PROJECT_SOURCE clean package -DskipTests
----

You should see `BUILD_SUCCESS`. If you get any errors, double-check that you made all of the correct changes to the correct files and try the build again.

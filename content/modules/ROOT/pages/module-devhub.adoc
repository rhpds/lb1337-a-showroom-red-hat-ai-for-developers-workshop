...
quarkus.langchain4j.openai.parasol-email.base-url={parasol-instruct-model-url}/v1
----

You also need to fix the issue in the previous deployment failure. The root case is that the application refers to the wrong LLM model. 
`Update` the *parasol-chat.base-url* to the following base URL in the `application.properties` file.

[.console-input]
[source,properties,subs="+attributes,macros+"]
----
quarkus.langchain4j.openai.parasol-chat.base-url={parasol-chat-model-url}/v1
----
...
image::devhub/icon_source_control.png[width=40%, float=left]
...
=== Access the Gitlab to review the changes

Go to the [Gitlab^] and review the changes.

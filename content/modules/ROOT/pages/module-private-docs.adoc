...
This Quarkus application is a customer service processing tool that handles customer claim emails for Parasol insurance. The team has recently improved the application with a chatbot to interact with the agent and to generate responses based on the email content. In the VS Code environment, navigate to the `app/src/main/java/org/parasol` directory, which contains the main source code of the application.
...
// TODO: add screenshot of application.properties once we have the right settings
...
We have already added the policy documents in the app/src/main/resources/catalog folder. Now we need to tell the Easy Rag extension where to find them by adding the following line to your app/src/main/resources/application.properties:
...
That's all there is to it! Now when we reload the application, it will parse the documents and vectorize them into an in-memory embedding store that will be used by the AI calls to the LLM.

NOTE: Of course this is a simple use case with a few text documents loaded into an in-memory embedding store. https://docs.quarkiverse.io/quarkus-langchain4j/dev/easy-rag.html[Click here] If you would like to learn more about Easy Rag and embedding models 

Let's reload the browser window to make sure Quarkus Dev mode picks up the changes, and then ask the same questions in the chat window. This time the bot should be able to answer the questions.
